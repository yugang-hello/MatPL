...
PairMATPL::~PairMATPL()
{
  ...
  for (int ff_idx = 1; ff_idx <= num_ff; ff_idx++) {
    ...
    if (imodel == 3) nn_ff_deallocate(&ff_idx);
...
void PairMATPL::allocate()
{
  ...
  memory->create(setflag, n, n, "pair_matpl:setflag");

  for (int i = 1; i < n; i++)
    for (int j = i; j < n; j++) setflag[i][j] = 0;

  memory->create(cut, n, n, "pair_matpl:cut");
  memory->create(cutsq, n, n, "pair_matpl:cutsq");

  memory->create(e_atom, natoms, "pair_matpl:e_atom");

  memory->create(num_neigh, natoms, ntypes, "pair_matpl:num_neigh");
  memory->create(list_neigh, natoms, ntypes, max_neigh_num, "pair_matpl:list_neigh");
  memory->create(dR_neigh, natoms, ntypes, max_neigh_num, 3, "pair_matpl:dR_neigh");
...
void PairMATPL::coeff(int narg, char **arg)
{
  ...
  // if (imodel != 5)
  //   error->all(FLERR, "pair_matpl only support imodel=5");
  ...
  memory->create(itype_atom, nmax, "pair_matpl:itype_atom");
...
{
  if (force->newton_pair == 0) error->all(FLERR, "Pair style MATPL requires newon pair on");
...
void PairMATPL::grow_memory()
{
  if (atom->nmax > nmax) {
    ...
    memory->grow(itype_atom, nmax, "pair_matpl:itype_atom");
  }
}

void PairMATPL::generate_neighdata()
{
  ...
  if (min_dR_all < 0.81) {
    ...
    error->universe_all(FLERR, "@@@ pair_matpl min_dR_all.");
  }
...
double PairMATPL::calc_max_f_err(double ***f_n)
{
  int i, j;
  double max_err, err;
  int ff_idx;
...
void PairMATPL::write_config(atom_config &config, int idx, int timestep)
{
  MPI_Status status;
  MPI_Request request;
  int nprocs, iproc;
